...
class Downloader
{
    ...
    static function get($url, $save_file, $speed = 10240, $headers = array(), $timeout = 10)
    {
        ...
        $headers = array_merge($def_headers,
            $headers);
        // get content length
        $content_length = self::getContentSize(
        ...
        try {
            $download_status = self::downloadContent(
                $url_info['host'], $url_info['port'],
     ...
     * @return int
     */
    static function getContentSize($host, $port, $url_path, &$headers, $timeout)
    {
        $request = self::buildHeader('HEAD', $url_path, $headers);
        ...
        while (!feof($fsocket)) {
            ...
            if (substr($line, 0, 9) == 'location:') {
                ...
                $url_info = self::parseUrl($location);
                if (!$url_info['host']) {
                    return 0;
                }
                fclose($fsocket);
                return self::getContentSize($url_info['host'], $url_info['port'], $url_info['request'], $headers,
                    $timeout);
            }
            // get content length
     ...
     * @return string
     */
    static function buildHeader($action, $url_path, &$headers, $range_start = -1)
    {
     ...
     * @param $timeout
     */
    static function downloadContent($host, $port, $url_path, $save_file, $content_length, $range_start, $speed, &$headers, $timeout) {
        $request = self::buildHeader('GET', $url_path, $headers, $range_start);
        $fsocket = @fsockopen($host, $port, $errno, $errstr, $timeout);
        stream_set_blocking($fsocket, true);
        stream_set_timeout($fsocket, $timeout);
        fwrite($fsocket, $request);
