...
回答：表单信息都可以通过这两个方法将数据传递到服务器上，GET 方法，所有的信息都会出现在 URL 地址中，并且使用 GET 方法最多只能传递 1024 个字符，所以如果在传输量小或者安全性不那么重要的情况下可以使用 GET 方法。POST 方法，最多可以传输 2MB 字节的数据，而且可以根据需要调节。
...
如果我们不想用 submit 按钮来提交表单，我们也可以用超链接来提交，我们可以这样写代码： `<a href=”javascript: document.myform.submit();”>Submit Me</a>`
