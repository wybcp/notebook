...
- 查找：因为链表中的数据并非连续存储的，所以无法像数组那样，根据首地址和下标，通过寻址公式就能直接计算出对应的内存地址，而是需要根据指针一个结点一个结点地依次遍历，直到找到相应的结点，O(n) 的时间复杂度。
- 插入和删除：只需要考虑相邻结点的指针改变
...
图中的元素叫作顶点（vertex），图中的一个顶点可以与任意其他顶点建立连接关系，叫作边（edge）。跟顶点相连接的边的条数，叫作顶点的度（degree）。
